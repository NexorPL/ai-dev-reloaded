    ...
    public async Task<Contracts.OpenAi.Completions.Response> CompletionsAsync(List<Contracts.OpenAi.Completions.Message> messages, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (result.IsSuccessStatusCode)
            {
                ...
                _logger.LogInformation("Completions - Response received from {0}: {1}", uri, content);
            ...
            {
                _logger.LogInformation($"Completions - Failed");
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Completions - error occured");
            throw;
        }

        throw new MissingCompletionsException();
    }
